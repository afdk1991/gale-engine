...
当前版本：**v0.3.0**（深度优化里程碑，七大功能模块全部可用）
...
- **主进程**：`electron/main.ts` 注册 29 个 IPC handler，`electron/preload.ts` 桥接为 `window.gale`（`contextIsolation: true`）
...
│   └── services/        # monitor / process / optimizer / gamemode / toolbox / network / history / settings / autolaunch / update
...
npm test                 # vitest 全量单测（当前 100 项全绿）
```
...
- **单测**：`npm test`（100 项，覆盖全部 service 纯逻辑 + 主题系统 + 侧边栏组件）
- **类型**：`npm run typecheck`
- **核心命令直测**：服务层 PowerShell 命令可在本机 PowerShell 直接验证（进程采集 / 挂起恢复 / Ping / 网卡 / 浏览器缓存扫描）
